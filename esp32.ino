...
//  setCpuFrequencyMhz(80);
  ...
  ledcSetup(0, 100, 8);         //1000Hz, CCR up to 256
  ...
  ledcSetup(1, 1000, 8);          //1000Hz, CCR up to 256
  ledcSetup(2, 1000, 8);
  ledcSetup(3, 1000, 8);
...
void distance (void){
  ...
  if(H_dist <= min_dist || L_dist <= min_dist)     //if not detected or user's back touched to back
    Diff_dist = 3.0;
...
void updateGoogleSheet(uint32_t value) {
  ...
  String addUrl = "https://sheets.googleapis.com/v4/spreadsheets/" + String(sheetId) + "/values/" + sheetName + "!" + getColumnLetter(lastCol) + String(lastRow);
